...
function Card(props: Props) {
  ...
  const twitterClassAccount = twitterAccounts.find((account) => account.loginNameTwitter === twitterAccount);
      ...
      <div id={twitterAccount} className='in-view flex flex-col w-full  '>
          ...
          <div className={`flex flex-col items-start space-x-5 ${BORDER_OUTSIDE_STYLING} p-3 w-72 max-w-full sm:w-52 md:w-60`}>
            ...
            <div className='Card-automation-container flex flex-row w-full space-x-5 py-1 items-center'>
              <button
                type='button'
                onClick={openTooltip}
                className='text-xs sm:text-sm bg-secondary p-1 font-semibold px-2 rounded-sm shadow-md hover:bg-accent hover:text-white hover:shadow-xl'
              >
                open
              </button>
              <div className='group'>
                ...
                <button
                  className='text-xs sm:text-sm Card-automation-button h-full hidden transition-opacity duration-300 ease-in-out group-hover:block  bg-accent p-1 px-2 text-white rounded-sm shadow-md'
                  onClick={autommationSwitch}
                >
        ...
        <Popup ref={ref}>
          <div className='Card-popup-container fixed top-0 left-0 right-0 bottom-0 z-50 bg-background w-screen h-screen flex flex-col items-center overflow-y-scroll space-y-2 pb-5  text-white'>
            ...
            <a href={`#${twitterAccount}`} type='button' className={`Card closeButton font-semibold px-2 ${BUTTON_STYLING}`} onClick={closeTooltip}>
              close
            </a>
          </div>
        </Popup>
