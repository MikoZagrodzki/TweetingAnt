...
export const getTimeToTweetsByEmail = async (email: string) => {
  const body = {
    email: email,
  };
  try {
    const response = await requestApi(
      "https://tweetingantdb-9ezx.onrender.com/twitterClass/getTimeToTweetsByEmail",
      {
        method: "POST",
        headers: {
...
export default getTimeToTweetsByEmail;
